...
file.""")
parser.add_argument("filename", help="The path to the XLSX file to process.")
parser.add_argument("--sheet", default=0, help="Sheet in the XLSX to use. Default: first sheet.")
...
# Identify header row
header_row = df[df.applymap(lambda x: "Sector" in str(x)).any(axis=1)].index[0]
...
normalize_merged_cols(columns_list, 'Industry Group', 'industry_group_code', 'industry_group_name')
normalize_merged_cols(columns_list, 'Industry', 'industry_code', 'industry_name')
normalize_merged_cols(columns_list, 'Sub-Industry', 'sub_industry_code', 'sub_industry_name')
...
df['industry_group_code'] = df['industry_group_code'].fillna(method='ffill')
df['industry_group_name'] = df['industry_group_name'].fillna(method='ffill')
